...
config:
  ...
  executor: mock # mock (local), copilot-sdk (Copilot API), or claude-code (Claude subscription)
...
| `executor`          | string    | `copilot-sdk`     | Executor: `mock` (local, echoes task metadata and file content), `copilot-sdk` (Copilot API), or `claude-code` (Claude Code CLI on a Claude subscription — see [schema reference](../../reference/schema/#executor)) |
